  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm text-muted-foreground">
        Showing {start}–{end} of {total.toLocaleString()} bookmarked tools
      </p>

      <div className="flex items-center gap-2">
        <Button variant="outline" size="sm" asChild disabled={!prevPage}>
          {prevPage ? (
            <Link href={buildBookmarksHref(filters, { page: prevPage })}>
        ...
        <Button variant="outline" size="sm" asChild disabled={!nextPage}>
          {nextPage ? (
            <Link href={buildBookmarksHref(filters, { page: nextPage })}>
              Next
